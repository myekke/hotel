...
public class Gym {
    public Gym(Connection conn) {
        JFrame gymform = new JFrame("Gym");
        gymform.setSize(400, 350);
        ...
        sendmessagebtn.setBounds(310, 280, 30, 30);

        ImageIcon water2 = new ImageIcon("recieve.png");
        JButton messagebtn = new JButton(water2);
        messagebtn.setBounds(350, 280, 30, 30);


        ActionListener actionListener1 = e -> {
            try {
                Statement statement = conn.createStatement();
                ResultSet resultSet = statement.executeQuery("SELECT * FROM Message where Getteruser = 'gymofhotel';");
        ...
        ActionListener actionListener2 = e -> {
            try {
                String insertSQL = "INSERT INTO GymTable (RoomNumber, name,lastname  ,cost , useday , usemonth ,useyear , gymindex ) VALUES ( ? ,? ,?, ? , ? ,?, ? , ? );";
        ...
        try {
            ...
            rs6.close();
            income.setText(String.valueOf(gymcost));
            income.setEnabled(false);
        } catch (SQLException e) {
            e.printStackTrace();
        }

        ActionListener actionListener3 = e -> new RequestForStaff(conn, "Gym");
        sendmessagebtn.addActionListener(actionListener3);

        gymform.add(messagebtn);
        gymform.add(sendmessagebtn);
        gymform.add(index);
        gymform.add(indexl);
        gymform.add(name);
        gymform.add(namel);
        gymform.add(lastname);
        gymform.add(lastnamel);
        gymform.add(datel);
        gymform.add(day);
        gymform.add(month);
        gymform.add(year);
        ...
        gymform.add(cost);
        gymform.add(costl);
        gymform.add(continuebtn);
        gymform.add(roomnumber);
        gymform.add(roomnumberl);
        gymform.setLocationRelativeTo(null);
        gymform.setResizable(false);
        gymform.setLayout(null);
        gymform.setVisible(true);
    }
}
