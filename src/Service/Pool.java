...
public class Pool {
    public Pool(Connection conn) {
        JFrame poolform = new JFrame("Pool");
        poolform.setSize(400, 350);
        ...
        ActionListener actionListener3 = e -> new RequestForStaff(conn, "Pool");
        ...
        ActionListener actionListener1 = e -> {
            try {
                ...
                ResultSet resultSet = statement.executeQuery("SELECT * FROM Message where Getteruser = 'poolofhotel';");
        ...
        ActionListener actionListener2 = e -> {
            try {
                String insertSQL = "INSERT INTO PoolTable ( RoomNumber, name,lastname  ,cost , useday , usemonth ,useyear , poolindex ) VALUES (?, ? , ? , ? ,? ,? ,?, ? );";
        ...
        try {
            ...
            rs6.close();
            income.setText(String.valueOf(poolcost));
            income.setEnabled(false);
        } catch (SQLException e) {
            e.printStackTrace();
        }

        poolform.add(sendmessagebtn);
        poolform.add(messagebtn);
        poolform.add(index);
        poolform.add(indexl);
        poolform.add(name);
        poolform.add(namel);
        poolform.add(lastname);
        poolform.add(lastnamel);
        poolform.add(datel);
        poolform.add(day);
        poolform.add(month);
        poolform.add(year);
        poolform.add(income);
        poolform.add(incomel);
        poolform.add(cost);
        poolform.add(costl);
        poolform.add(continuebtn);
        poolform.add(roomnumber);
        poolform.add(roomnumberl);
        poolform.setLocationRelativeTo(null);
        poolform.setResizable(false);
        poolform.setLayout(null);
        poolform.setVisible(true);
    }
}
